...
每天北京时间 09:30 左右运行（GitHub Actions 可能有少量延迟），中文模式会生成四份独立材料：

1. `YYYY-MM-DD-all-candidates-zh.md`：当天去重后的全部候选资讯，保留标题、原始链接、来源、双端评分与入选状态；
2. `YYYY-MM-DD-low-score-watchlist-zh.md`：两端均未入选且评分低于 6 分的资讯，只保留标题、链接和简要概述；
3. `YYYY-MM-DD-parent-topics-zh.md`：家长端 3—5 条重点选题；
4. `YYYY-MM-DD-teacher-topics-zh.md`：教师端 3—5 条重点选题，也是邮件、Webhook 与旧版摘要入口的默认内容。
